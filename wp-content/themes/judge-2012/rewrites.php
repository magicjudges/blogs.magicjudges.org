...
function cets_fyi_theme_template($arg){
	...
	global $wp_query;
		...
		// if this is a category page, get out of here
	if (isset($wp_query->query_vars['category_name']) && strlen($wp_query->query_vars['category_name']) > 0){
		return $arg;
	}
	...
	if (isset($wp_query->query_vars['topic'])) {
			// explode the query_vars so we can look for feed
			$topicvar = explode("/", $wp_query->query_vars['topic']);
			//if there's a "slash" in the topic var and this isn't a feed, get out of here otherwise, process topic.php
			if (stripos($wp_query->query_vars['topic'], "/") === false || (isset($topicvar[1]) && $topicvar[1] == 'feed')) {
		...
		if (stripos($wp_query->query_vars['sitelist'], "/") === false) {
