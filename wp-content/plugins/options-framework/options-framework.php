...
function optionsframework_validate( $input ) {
	...
	foreach ( $options as $option ) {
		...
		/* BEGIN CETS CAPABILITY RESTRICTION */

		if (isset($option['capability']) && $option['capability']) {
			if (!current_user_can($option['capability'])) {
				// Input is currently submitted values.
				// options is full options definition
				$option_id = $option['id'];
				$clean[$option['id']] = of_get_option($option['id']);
				continue;
			}
		}

		/* END CETS CAPABILITY RESTRICTION */
...
function of_get_default_values() {
	...
	foreach ( (array) $config as $option ) {
		...
		// After loading std, verify that we should've done that.
		if (isset($option['capability']) && $option['capability']) {
			if (!current_user_can($option['capability'])) {
